...
class Slideshow {
    ...
    DOM = {
        ...
        mainTitleTexts: [
            ...document.querySelectorAll(".title > .hfz > .hfz__inner")
        ],
	...
	*/ initEvents() {
        ...
        document.addEventListener("keydown", (event)=>{
            if (this.isOpen && !this.isAnimating) {
                if (event.key === "Escape") // Close the slideshow on pressing the "Escape" key
                this.close();
                else if (event.key === "ArrowUp") // Navigate to the previous item on pressing the up arrow key
                this.navigate("prev");
                else if (event.key === "ArrowDown") // Navigate to the next item on pressing the down arrow key
                this.navigate("next");
            }
        });
        // Define a scrollDown function to handle scrolling down
        const scrollDown = ()=>{
            if (this.isOpen && !this.isAnimating) this.navigate("prev"); // Scroll down
        };
        // Define a scrollUp function to handle scrolling up
        const scrollUp = ()=>{
            if (this.isOpen && !this.isAnimating) this.navigate("next"); // Scroll up
        };
...
class ContentItem {
    ...
    DOM = {
        ...
        description: null,
        // all .hfz__inner text spans that will animate in/out
        texts: null
    };
    /**
	 * Constructor.
	 ...
	 */ constructor(DOM_el){
        ...
        this.DOM.texts = [
            ...this.DOM.el.querySelectorAll(".hfz > .hfz__inner")
        ];
    }
}
